...
void wz_dileptonMetAnalysis() {
  ...
  addVariable( "thirdMuPt",         "Third muon p_{T} [GeV/c]",                 "", 60,  0.,    60.,         false, false );
  addVariable( "thirdEPt",          "Third electron p_{T} [GeV/c]",             "", 60,  0.,    60.,         false, false );
  ...
  // WZ background
  addVariable( "extraLeptonGenId",  "3^{rd} lepton gen. ID",              "",           7,   9.5,  16.5, true,  false );
  addVariable( "extraMuonGenPt",    "3^{rd} muon gen. p_{T} [GeV/c]",     "",           20,  0.,   100., true,  false );
  addVariable( "extraMuonEffPt",    "3^{rd} muon gen. p_{T} [GeV/c]",     "Efficiency", 20,  0.,   100., true,  false );
  addVariable( "extraMuonGenEta",   "3^{rd} muon gen. #eta",              "",           24,  -6.,  6.,   true,  false );
  addVariable( "extraMuonEffEta",   "3^{rd} muon gen. #eta",              "Efficiency", 24,  -6.,  6.,   true,  false );
  addVariable( "extraEleGenPt",     "3^{rd} electron gen. p_{T} [GeV/c]", "",           20,  0.,   100., true,  false );
  addVariable( "extraEleEffPt",     "3^{rd} electron gen. p_{T} [GeV/c]", "Efficiency", 20,  0.,   100., true,  false );
  addVariable( "extraEleGenEta",    "3^{rd} electron gen. #eta",          "",           24,  -6.,  6.,   true,  false );
  addVariable( "extraEleEffEta",    "3^{rd} electron gen. #eta",          "Efficiency", 24,  -6.,  6.,   true,  false );

  addVariable( "extraLeptonGenIdEM","3^{rd} lepton gen. ID e-#mu",        "",           1,   0.,   2.,   false, true  );
  addVariable( "extraLeptonGenIdAll","3^{rd} lepton gen. ID all",         "",           1,   0.,   2.,   false, true  );
  addVariable( "extraMuonGenAll",   "3^{rd} muon gen.",                   "",           1,   0.,   2.,   false, true  );
  addVariable( "extraMuonGenAcc",   "3^{rd} muon gen. accept.",           "",           1,   0.,   2.,   false, true  );
  addVariable( "extraEleGenAll",    "3^{rd} electron gen.",               "",           1,   0.,   2.,   false, true  );
  ...
  */
  ...
  if(doPlot) {
    ...
    */
    ...
    // 
    // double var_wz_mm_finalYield(0.), err_wz_mm_finalYield(0.); 
    // double var_wz_ee_finalYield(0.), err_wz_ee_finalYield(0.); 

    // var_wz_mm_finalYield = allhistos["h_wz_mm_finalYield"]->IntegralAndError(0, -1, err_wz_mm_finalYield);
    // var_wz_ee_finalYield = allhistos["h_wz_ee_finalYield"]->IntegralAndError(0, -1, err_wz_ee_finalYield);
...
void fillPlots(std::map<TString, TH1F*> & histos, TString plotlab, double wght) {
  ...
  // addVariable( "dileptPt",          "Dilepton p_{T} [GeV/c]",                   "", 60,  0.,    120.,        false, true  );
  // addVariable( "jetNumber",         "PFJet number (p_{T} > 30 GeV/c)",          "", 6,   -0.5,  5.5,         false, true  );
  // //addVariable( "cmsIndMinRedMet",   "CMS reduced MET [GeV]",                    "", 100, 0.,    300.,        false, true  ); 
  // addVariable( "d0RedMet",          "D0 reduced MET [GeV]",                     "", 100, 0.,    300.,        false, false ); 
  // addVariable( "metPtBalance",      "PF MET/p_{T}(Z)",                          "", 30,  0.,    3.,          false, true  ); 
  // addVariable( "deltaPhiJetMet",    "#Delta#phi(jet,MET) [rad]",                "", 18., 0.,    3.141592654, false, true  ); 
  // addVariable( "jetCsv",            "CSV discriminator (PFJet p_{T} > 20 GeV)", "", 22,  -1.1,  1.1,         false, true  );
  // addVariable( "thirdMuPt",         "Third muon p_{T} [GeV/c]",                 "", 60,  0.,    60.,         false, false );
  // addVariable( "thirdEPt",          "Third electron p_{T} [GeV/c]",             "", 60,  0.,    60.,         false, false );
  // //addVariable( "extraLeptonNumber", "Additional lepton number (loose sel.)",    "", 4,   -0.5,  3.5,         false, true  );
  ...
  // Useful WZ plots
  if( plotlab.Contains("_wz_") ) {

    UInt_t idxGenThird = 0;
    if( abs(mc_id[0])==24 )      idxGenThird = 1;
    else if( abs(mc_id[3])==24 ) idxGenThird = 4;
    else {}

    Int_t thirdGenId = abs(mc_id[idxGenThird]);
    Float_t thirdGenPt = getPt( mc_px[idxGenThird], mc_py[idxGenThird] );
    Float_t thirdGenEta = getEta( mc_px[idxGenThird], mc_py[idxGenThird], mc_pz[idxGenThird] );
    Float_t thirdGenPhi = getPhi( mc_px[idxGenThird], mc_py[idxGenThird] );

    histos[plotlab+"extraLeptonGenId"]->Fill(thirdGenId, wght);
    histos[plotlab+"extraLeptonGenIdAll"]->Fill(1., wght);

    if( thirdGenId==13 ) {
      histos[plotlab+"extraLeptonGenIdEM"]->Fill(1., wght);

      // Acceptance
      histos[plotlab+"extraMuonGenAll"]->Fill(1., wght);
      if( thirdGenPt>10. && fabs(thirdGenEta)<2.4 ) {
	histos[plotlab+"extraMuonGenAcc"]->Fill(1., wght);

	// Efficiency
	histos[plotlab+"extraMuonGenPt"]->Fill(thirdGenPt, wght);
	histos[plotlab+"extraMuonGenEta"]->Fill(thirdGenEta, wght);

	for(Int_t li=0; li<ln; ++li) {
	  if( abs(ln_id[li])!=13 ) continue;
	  Float_t recEta = getEta( ln_px[li], ln_py[li], ln_pz[li] );
	  Float_t recPhi = getPhi( ln_px[li], ln_py[li] );
	  if( fabs(recEta-thirdGenEta)>0.2 || getDeltaPhi(recPhi, thirdGenPhi)>0.2 ) continue;
	  histos[plotlab+"extraMuonEffPt"]->Fill(thirdGenPt, wght); 
	  histos[plotlab+"extraMuonEffEta"]->Fill(thirdGenEta, wght); 
	  break;
	}
      }
    ...
    else if( thirdGenId==11 ) {
      histos[plotlab+"extraLeptonGenIdEM"]->Fill(1., wght);

      // Acceptance
      histos[plotlab+"extraEleGenAll"]->Fill(1., wght);
      if( thirdGenPt>10. && fabs(thirdGenEta)<2.5 ) {
	histos[plotlab+"extraEleGenAcc"]->Fill(1., wght);

	// Efficiency
	histos[plotlab+"extraEleGenPt"]->Fill(thirdGenPt, wght);
	histos[plotlab+"extraEleGenEta"]->Fill(thirdGenEta, wght);
	for(Int_t li=0; li<ln; ++li) {
	  if( abs(ln_id[li])!=11 ) continue;
	  Float_t recEta = getEta( ln_px[li], ln_py[li], ln_pz[li] );
	  Float_t recPhi = getPhi( ln_px[li], ln_py[li] );
	  if( fabs(recEta-thirdGenEta)>0.2 || getDeltaPhi(recPhi, thirdGenPhi)>0.2 ) continue;
	  histos[plotlab+"extraEleEffPt"]->Fill(thirdGenPt, wght); 
	  histos[plotlab+"extraEleEffEta"]->Fill(thirdGenEta, wght); 
	  break;
	}
      }
  ...
  histos[plotlab+"finalYield"]->Fill( 1., wght*extraFact );
...
void printEvents(std::map<int, TString> & finst, 
  ...
  for(std::map<int, TString>::iterator fs=finst.begin(); fs!=finst.end(); ++fs) {
    ...
    for(unsigned int j=0; j<nhiggslabs; ++j) {
      ...
      // 
      for(unsigned int h=0; h<allVarsToPrint.size(); ++h) {
	std::cout << "\t" << higgshistos[plotlab+allVarsToPrint[h]]->Integral(0, -1);
      }
    }
	...
	stoptop = true;
      ...
      for(unsigned int h=0; h<allVarsToPrint.size(); ++h) {
	double tmpError = 0.0;
	std::cout << "\t" << histos[plotlab+allVarsToPrint[h]]->IntegralAndError(0, -1, tmpError);
	//std::cout << " +- " << tmpError;
      }
    }
    ...
    // 
    {
      ...
      // 
      for(unsigned int h=0; h<allVarsToPrint.size(); ++h) {
	double tmpMcError = 0.0;
	std::cout << "\t" << tothistos[plotlab+allVarsToPrint[h]]->IntegralAndError(0, -1, tmpMcError);
	std::cout << " +- " << tmpMcError;
      }
    }
    ...
    // 
    {
      ...
      // 
      for(unsigned int h=0; h<allVarsToPrint.size(); ++h) {
	double tmpDtError = 0.0;
	std::cout << "\t" << datahistos[plotlab+allVarsToPrint[h]]->IntegralAndError(0, -1, tmpDtError);
	std::cout << " +- " << tmpDtError;
      }
    }
