...
Int_t   ajnum;
Float_t ajn_px[maxjn];
Float_t ajn_py[maxjn];
Float_t ajn_pz[maxjn];
Float_t ajn_en[maxjn];
Float_t ajn_genpt[maxjn];
Float_t ajn_btag1[maxjn];
Float_t ajn_btag2[maxjn];
Bool_t  ajn_tightId[maxjn];
Float_t htvec_px;
Float_t htvec_py;
...
void initializeTreeVariables() {
  ...
  jnum = 0;
  for(unsigned int i=0; i<maxjn; ++i) {
    jn_px[i] = 0.;
    jn_py[i] = 0.;
    jn_pz[i] = 0.;
    jn_en[i] = 0.;
  ...
  ajnum = 0;
  for(unsigned int i=0; i<maxajn; ++i) {
    ajn_px[i] = 0.;
    ajn_py[i] = 0.;
    ajn_pz[i] = 0.;
    ajn_en[i] = 0.;
...
void attachToTree(TChain *t) {
  ...
  t->SetBranchAddress("ajn", &ajnum);
  t->SetBranchAddress("ajn_px", &ajn_px);
  t->SetBranchAddress("ajn_py", &ajn_py);
  t->SetBranchAddress("ajn_pz", &ajn_pz);
  t->SetBranchAddress("ajn_en", &ajn_en);
  t->SetBranchAddress("ajn_genpt", &ajn_genpt);
  t->SetBranchAddress("ajn_btag1", &ajn_btag1);
  t->SetBranchAddress("ajn_btag2", &ajn_btag2);
  t->SetBranchAddress("ajn_tightId", &ajn_tightId);
