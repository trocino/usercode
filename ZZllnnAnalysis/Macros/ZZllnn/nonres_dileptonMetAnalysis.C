...
void nonres_dileptonMetAnalysis() {
  ...
  addVariable( "dileptMetTransMass",        "Dilepton-MET transverse mass [GeV/c^{2}]",     "",  20, 0. , 400., true, false );
  addVariable( "dileptMetTransMassAlpha",   "Dilepton-MET transverse mass [GeV/c^{2}]",     "",  20, 0. , 400., true, false );
  addVariable( "leadLMetTransMass",         "Leading l-MET transverse mass [GeV/c^{2}]",    "",  20, 0. , 400., true, false );
  addVariable( "leadLMetTransMassAlpha",    "Leading l-MET transverse mass [GeV/c^{2}]",    "",  20, 0. , 400., true, false );
  addVariable( "subleadLMetTransMass",      "Subleading l-MET transverse mass [GeV/c^{2}]", "",  20, 0. , 400., true, false );
  addVariable( "subleadLMetTransMassAlpha", "Subleading l-MET transverse mass [GeV/c^{2}]", "",  20, 0. , 400., true, false );
...
void fillPlots(std::map<TString, TH1F*> & histos, TString plotlab, double wght, double puWght) {
  ...
  //Float_t thisCMSredMet = getCMSRedMet(l1_px, l1_py, 0., l2_px, l2_py, 0., htvec_px, htvec_py, met_pt[0], met_phi[0], redMetCompts, cat); // no lept. uncert.
  ...
  bool dileptMetTransMassCut( thisDileptMetTransMass>140. ); 

  // Same sign (against W?)
  bool sameSignBool( l1_id*l2_id<0 );

  bool peakBool(false);
  bool sideBool(false);

  //if( fabs(thismass-91.)<10. ) {  // fabs(thismass-91.1876)
  if( fabs(thismass-91.)<7. ) {  // fabs(thismass-91.1876)
    peakBool = true;
  }
