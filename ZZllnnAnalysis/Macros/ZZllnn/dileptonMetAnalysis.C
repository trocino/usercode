...
void dileptonMetAnalysis() {
  ...
  // Pedro's colors
  Color_t cols[]={1, 1, 1, 1, 590, 596, 592, 8, 824, 824, 824, 824, 824, 824, 834, 809, 831}; // WARNING: 834 is assigned to ZZX here, but belongs to DYToTauTau
  ...
  allfiles["zh105"].push_back(higgsbase_mc+"ZH150.root"); //"ZH105.root");
  allfiles["zh115"].push_back(higgsbase_mc+"ZH150.root"); //"ZH115.root");
  allfiles["zh125"].push_back(higgsbase_mc+"ZH150.root"); //"ZH125.root");
  ...
  addVariable( "pfMet",             "PF MET [GeV]",                             "", 100, 0.,    300.,        true,  false ); 
  ...
  addVariable( "largeDileptMass",   "Dilepton invariant mass [GeV/c^{2}]",      "", 8,   40.,   200.,        true,  false );
  ...
  addVariable( "ALLpfMet",             "PF MET [GeV]",                             "", 100, 0.,    300.,        true,  false ); 
  ...
  addVariable( "ALLlargeDileptMass",   "Dilepton invariant mass [GeV/c^{2}]",      "", 8,   40.,   200.,        true,  false );
  ...
  if(doPlot) {
    ...
    std::map<TString, TPaveText*> allpavetexts;
    ...
    // Text for TPaveText
    char Buffer[1024]; sprintf(Buffer, "CMS preliminary, #sqrt{s}=7 TeV, #int L=%.1f fb^{-1}", intLumi/1000.);
	...
	allpavetexts[plotIdx] = new TPaveText(0.40,0.995,0.85,0.945, "NDC");
	allpavetexts[plotIdx]->SetFillColor(0);
	allpavetexts[plotIdx]->SetFillStyle(0); 
	allpavetexts[plotIdx]->SetLineColor(0);
	allpavetexts[plotIdx]->SetTextAlign(32);
	allpavetexts[plotIdx]->AddText(Buffer);
      }
    }
	...
	allpavetexts[fsAndVar]->Draw("same");
	...
	if(binbybinComp && drawData) {	// Start bin-by-bin data-MC comparison
	  ...
	  allfracdiffhistos[fsAndVar] = (TH1F*)allmchistos["h_sumAllMc_"+fsAndVar]->Clone( ("h_fracDiff_"+fsAndVar).Data() );
	  ...
	  //allfracdiffhistos[fsAndVar]->SetMarkerSize(0.8);   // comment out for Pedro's style
	  //allfracdiffhistos[fsAndVar]->SetMarkerColor(kRed); // comment out for Pedro's style
	  //allfracdiffhistos[fsAndVar]->SetLineColor(kRed);   // comment out for Pedro's style
	  ...
	  allpads3[fsAndVar]->Modified();
	  allpads3[fsAndVar]->Update();
...
void fillPlots(std::map<TString, TH1F*> & histos, TString plotlab, double wght, double puWght) {
  ...
  if( isMC==false ) {
	...
	l1_px *= enCorr1; l1_py *= enCorr1; l1_pz *= enCorr1; l1_en *= enCorr1; 
      ...
      else if( abs(l2_id)==11 ) {
	double enCorr2 = en_corren[l2_pid]/l2_en; if(enCorr2==0) enCorr2 = 1.0;
	l2_px *= enCorr2; l2_py *= enCorr2; l2_pz *= enCorr2; l2_en *= enCorr2; 
      } 
      else {
	std::cout << " ******************* ERROR!!! *******************" << std::endl;
	std::cout << "  Category 3, but neither l1 nor l2 has |id| 11!"  << std::endl;
  ...
  // Full weight (for cut-flow plot/table)
  if( allVarsToPrintBins.count("preselWeights")>0 ) histos[plotlab+"cutFlow"]->Fill( float(allVarsToPrintBins["preselWeights"]), wght); 
  ...
  histos[plotlab+"ALLlargeDileptMass"]->Fill(thismass, wght);
  histos[plotlab+"ALLdileptMass"]->Fill(thismass, wght);
  ...
  histos[plotlab+"ALLdileptPt"]->Fill(zPt, wght);
  if(peakBool) {
  ...
  histos[plotlab+"ALLjetCsv"]->Fill(thisbtagval, wght);
  if(peakBool && zPtBool) {
  ...
  //std::vector<UInt_t> jets30 = getListOfParticlesWithPt(use_jn, use_jn_px, use_jn_py, 30., 0, 0, 0, use_jn_tightId); // WARNING: DY has no tightId (in Apr-14 samples)
  std::vector<UInt_t> jets30 = getListOfParticlesWithPt(use_jn, use_jn_px, use_jn_py, 30.); // tightId not required
  unsigned int jets30N = jets30.size();
  histos[plotlab+"ALLjetNumber"]->Fill(jets30N, wght);
  if(peakBool && zPtBool && csvBool) {
  ...
  //Float_t thisCMSredMet = getCMSRedMet(momSc1*l1_px, momSc1*l1_py, 0., momSc2*l2_px, momSc2*l2_py, 0.,     htvec_px,     htvec_py, met_pt[0], met_phi[0], redMetCompts, cat); // no lept. uncert.
  Float_t thisCMSredMet = getCMSRedMet(momSc1*l1_px, momSc1*l1_py, 0., momSc2*l2_px, momSc2*l2_py, 0., use_htvec_px, use_htvec_py, met_pt[0], met_phi[0], redMetCompts, cat); // no lept. uncert.
  histos[plotlab+"ALLcmsIndMinRedMet"]->Fill(thisCMSredMet, wght);
  histos[plotlab+"ALLpfMet"]->Fill(met_pt[0], wght);
  if(peakBool && zPtBool && csvBool && jets30NBool) {
    histos[plotlab+"cmsIndMinRedMet"]->Fill(thisCMSredMet, wght);
    histos[plotlab+"pfMet"]->Fill(met_pt[0], wght);
  }
  ...
  histos[plotlab+"ALLmetPtBalance"]->Fill(metPtBal, wght);
  if(peakBool && zPtBool && csvBool && jets30NBool && redMetBool) {
  ...
  histos[plotlab+"ALLdeltaPhiJetMet"]->Fill(dPhiJetMet, wght);
  if(peakBool && zPtBool && csvBool && jets30NBool && redMetBool && balBool) {
  ...
  //std::vector<UInt_t> thirdlept10 = getListOfParticlesWithPt(ln, ln_px, ln_py, 10., true, ln_id, ln_pz); // for lepton momentum scale
  unsigned int thirdlept10N = thirdlept10.size();
  histos[plotlab+"ALLextraLeptonNumber"]->Fill(thirdlept10N, wght);
  if(peakBool && zPtBool && csvBool && jets30NBool && redMetBool && balBool && jetMetPhiBool) {
    histos[plotlab+"extraLeptonNumber"]->Fill(thirdlept10N, wght);
  }
  bool thirdLeptBool(thirdlept10N==0);
  if(peakBool && zPtBool && csvBool && jets30NBool && redMetBool && balBool && jetMetPhiBool && thirdLeptBool) {
    ...
    double phi1 = getPhi(l1_px, l1_py);
    double phi2 = getPhi(l2_px, l2_py);
  ...
  if(/*peakBool &&*/ zPtBool && csvBool && jets30NBool && redMetBool && balBool && jetMetPhiBool && thirdLeptBool) 
    histos[plotlab+"largeDileptMass"]->Fill(thismass, wght);
...
void printEvents(std::map<int, TString> & finst, 
  ...
  for(std::map<int, TString>::iterator fs=finst.begin(); fs!=finst.end(); ++fs) {
    ...
    // Headers
    for(unsigned int h=0; h<allVarsToPrint.size(); ++h) {
      if(latexStyle) std::cout << " & " << allVarsToPrint[h];
      else           std::cout << "\t"  << allVarsToPrint[h];
    }
	...
	if(latexStyle) std::cout << " & " << tmpVal;
	else           std::cout << "\t"  << tmpVal;
      }
    }
	...
	int errOrd = approxToN(tmpErr, 1);
	approxToN(tmpVal, 1, errOrd);
	if(latexStyle) std::cout << " & " << tmpVal << " \\pm " << tmpErr;
	else           std::cout << "\t"  << tmpVal << " +- " << tmpErr;
      }
    }
