// -*- C++ -*-
//
// Package:    ME0MuonGeneralAnalyzer
// Class:      ME0MuonGeneralAnalyzer
// 
/**\class ME0MuonGeneralAnalyzer 
...
class ME0MuonGeneralAnalyzer : public edm::EDAnalyzer {
public:
  explicit ME0MuonGeneralAnalyzer(const edm::ParameterSet&);
  ~ME0MuonGeneralAnalyzer();
...
//
ME0MuonGeneralAnalyzer::ME0MuonGeneralAnalyzer(const edm::ParameterSet& iConfig) :
  ...
  //muSegmMapToken_( consumes<MuonSegmentMap>(iConfig.getParameter<edm::InputTag>("muSegmMapTag")) ) 
{
  ...
  runEfficiency = iConfig.getUntrackedParameter<bool>("runEfficiency", true ); 
  runFakes      = iConfig.getUntrackedParameter<bool>("runFakes"     , true ); 
...
ME0MuonGeneralAnalyzer::~ME0MuonGeneralAnalyzer() { 
...
void ME0MuonGeneralAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  using std::string;
  ...
  */
  ...
  if(runFakes) { 
    for(unsigned int h=0; h<(*trackColl).size(); ++h) { 
      ...
      if(thisTrack->p()>minP || thisTrack->pt()>minPt) { 
	// hists_["fake_den_eta" ]->Fill( thisTrack->eta() );
...
void ME0MuonGeneralAnalyzer::beginJob() {
}

// ------------ method called once each job just after ending the event loop  ------------
void ME0MuonGeneralAnalyzer::endJob() { 
...
void ME0MuonGeneralAnalyzer::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup) {
...
void ME0MuonGeneralAnalyzer::endRun(edm::Run const&, edm::EventSetup const&) {
}


// ------------ method called when starting to processes a luminosity block  ------------
void ME0MuonGeneralAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}

// ------------ method called when ending the processing of a luminosity block  ------------
void ME0MuonGeneralAnalyzer::endLuminosityBlock(edm::LuminosityBlock const& iLumiBlock, edm::EventSetup const&) {
}


/*
bool ME0MuonGeneralAnalyzer::isGoodME0Muon(edm::ESHandle<ME0Geometry> me0Geom, const reco::ME0Muon& me0muon, double MaxPullX, double MaxDiffX, double MaxPullY, double MaxDiffY, double MaxDiffPhiDir )
{
...
DEFINE_FWK_MODULE(ME0MuonGeneralAnalyzer);
